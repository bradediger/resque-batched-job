  ...
  module Plugins
    ...
    module BatchedJob
      ...
      def enqueue_batch(batch_id, arglist)
        ...
        arglist.each { |args| Resque.redis.sadd(batch(batch_id), Resque.encode(args)) }
      ...
      def after_enqueue_batch(id, *args)
        Resque.redis.sadd(batch(id), Resque.encode(args))
      end
  ...
  TODO: Determine if it's necessary to double check the jobs existance
    before performing it.  If so, do what?
      ...
      def before_perform_audit_batch(id, *args)
        unless Resque.redis.sismember(batch(id), "#{Resque.encode(args)}")
...
=end
      ...
      def around_perform_amend_batch(id, *args)
        begin
          ...
          Resque.redis.srem(batch(id), "#{Resque.encode(args)}")
        end
      end
